 ...
 */
export function VirtualRegister({
   clock = null,
   data_bus = null,
   control_bus = null,
   size = 8,
...
})
{
   ...
   clock.add_rising_client(() => {
      if(write_enable_idx >= 0 && this.control_bus.get(write_enable_idx))
         this.do_write();
      if(read_enable_idx >= 0 && this.control_bus.get(read_enable_idx))
         this.do_read();
   });
   this.clients = [];
}
...
{
   this.data_bus.set_register(this.register);
};
...
/**
 * Adds a client that gets called when the register is changed by reading from the bus
 * 
 * @param {function} client Specifies the callback function that will be called.
 */
VirtualRegister.prototype.add_client = function(client) 
{
